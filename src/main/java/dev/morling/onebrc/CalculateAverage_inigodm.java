...
public class CalculateAverage_inigodm {
    ...
    public static void main(String[] args) throws IOException {
        ...
        Map<String, ResultRow> measurements = new TreeMap<>(Files.lines(Paths.get(FILE), StandardCharsets.UTF_8)
                .parallel()
                //.map(l -> split(';', 0, l))
                .map(l -> l.split(";"))
    ...
    private static String[] split(char ch, int limit, String s) {
        int matchCount = 0;
        int off = 0;
        int next;
    ...
    private static Stream<String> createFileChannelLinesStream(FileChannel fc, Charset cs) throws IOException {
        try {
            ...
            if (length > 0) {
                dev.morling.onebrc.FileChannelLinesSpliterator fcls = new dev.morling.onebrc.FileChannelLinesSpliterator(fc, cs, 0, length);
                return StreamSupport.stream(fcls, true)
                        .onClose(() -> {
                            try {
                                fc.close();
                            }
    ...
    public static Stream<String> lines(BufferedReader br) {
        ...
        return StreamSupport.stream(Spliterators.spliteratorUnknownSize(
                iter, Spliterator.ORDERED | Spliterator.NONNULL), true);
    }
}

class FileChannelLinesSpliterator implements Spliterator<String> {
    ...
    FileChannelLinesSpliterator(FileChannel fc, Charset cs, long index, long fence) {
        this.fc = fc;
        this.cs = cs;
        this.index = index;
        this.fence = fence;
